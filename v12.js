...
async function ensureRadarMeta(force=false){
  ...
  try{const j=await(await apiFetch('/api/weather/radar')).json();radarMeta=j||{frames:[]};if(j.frames?.length){radarIndex=j.frames.length-1;$('#radarStatus').textContent=`HKO · ${j.frames[radarIndex].time||'latest'}`}else $('#radarStatus').textContent='HKO radar unavailable'}catch(e){radarMeta={frames:[]};$('#radarStatus').textContent='HKO radar unavailable'}return radarMeta;
}
async function showRadarFrame(){if(!geoMap||!radarOn)return;await ensureRadarMeta();if(!radarMeta.frames?.length)return;const f=radarMeta.frames[Math.max(0,Math.min(radarMeta.frames.length-1,radarIndex))];if(radarLayer)geoMap.removeLayer(radarLayer);radarLayer=L.imageOverlay(window.API+'/api/weather/radar-image?u='+encodeURIComponent(f.url),f.bounds,{opacity:(+$('#radarOpacity').value||55)/100,interactive:false,zIndex:250}).addTo(geoMap);$('#radarStatus').textContent=`HKO · ${f.time||`frame ${radarIndex+1}/${radarMeta.frames.length}`}`}
